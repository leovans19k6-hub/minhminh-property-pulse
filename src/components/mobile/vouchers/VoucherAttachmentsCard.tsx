...
import type { MobileVoucherAttachment } from "@/services/mobile/vouchers.service";
...
function iconFor(a: MobileVoucherAttachment) {
  const t = ((a.type as string | undefined) ?? (a.mime_type as string | undefined) ?? "").toLowerCase();
  if (t.startsWith("image") || t === "image") return ImageIcon;
  if (t === "pdf" || t.includes("pdf")) return FileText;
  return File;
}

export function VoucherAttachmentsCard({ attachments }: { attachments: MobileVoucherAttachment[] }) {
