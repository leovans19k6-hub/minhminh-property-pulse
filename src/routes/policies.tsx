...
import { useMobilePolicies } from "@/features/policies/queries";
...
export const Route = createFileRoute("/policies")({
  ...
  head: () => ({
    meta: [
      { title: "Chính sách bán hàng — Minh Minh Portal" },
      {
        name: "description",
        content:
          "Danh sách chính sách bán hàng đang áp dụng cho các dự án bạn có quyền truy cập.",
      },
    ],
  }),
});
...
function PoliciesPage() {
  ...
  return (
    <MobileShell showHeader={false}>
      <PageHeader title="Chính sách bán hàng" subtitle={subtitle} />
      ...
      <div className="space-y-3 p-4 pb-8">
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Tìm theo tên hoặc mô tả chính sách"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(items.length > 0 || isLoading) && (
            <span className="ml-auto text-[11px] text-[color:var(--text-tertiary)]">
              {isLoading ? "Đang tải…" : `${items.length} chính sách`}
            </span>
          )}
        </div>
        ...
        {isLoading ? (
          ...
          <EmptyBlock projectId={search.projectId ?? null} isSearching={!!(search.q || search.featured)} />
        ) : (
          <ul className="space-y-2.5">
            {items.map((it) => (
              <li key={it.id}>
                <MobilePolicyCard item={it} showProject={!search.projectId} />
              </li>
            ))}
          </ul>
        )}
...
function EmptyBlock({ projectId, isSearching }: { projectId: string | null; isSearching: boolean }) {
  if (isSearching) {
    return <MobileEmptyState title="Không tìm thấy chính sách phù hợp" />;
  }
  if (projectId) {
    return (
      <div className="space-y-3">
        <MobileEmptyState title="Chưa có chính sách đang áp dụng" />
  ...
  return (
    <div className="space-y-3">
      <MobileEmptyState title="Hiện chưa có chính sách đang áp dụng" />
      <div className="flex justify-center">
